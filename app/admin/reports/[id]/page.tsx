...
export default function MasterJobReport() {
  ...
  useEffect(() => {
    document.title = job ? `Report - ${job.client_name} - ${job.wo_number || job.legacy_id}` : 'Job Report';
  }, [job]);
  ...
  if (!job) {
    return (
      <div className="w-full h-full flex flex-col items-center justify-center font-mono">
        <h2 className="text-xl font-bold uppercase tracking-widest text-foreground">Job Not Found</h2>
        <button onClick={() => router.back()} className="mt-4 border border-border px-4 py-2 hover:bg-surface transition-colors">
          RETURN TO JOBS
        </button>
      </div>
    );
  ...
  const devOverridesRaw = typeof window !== 'undefined' ? sessionStorage.getItem('rsg-dev-overrides') : '{}';
  ...
  const fieldCapturesRaw = typeof window !== 'undefined' ? sessionStorage.getItem(`field_captures_${job.id}`) : '{}';
  ...
  const avgAccuracy = avg !== null 
    ? (avg <= 10 ? `HIGH ACCURACY (±${avg.toFixed(1)}M)` : `MODERATE (±${avg.toFixed(1)}M)`)
    : 'UNKNOWN';

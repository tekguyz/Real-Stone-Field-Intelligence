  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">{t.installationTeam}</h1>
          <p className="text-foreground/50 mt-1 font-mono text-sm leading-none">PERSONNEL & ACCESS CONTROL</p>
        </div>
        <button 
        ...
        {mockTeam.map(member => (
          ...
          >
            ...
            <div className="flex justify-between items-start mb-6">
              ...
              <div className={`px-2.5 py-1 text-[10px] font-bold uppercase tracking-widest ${
                member.status === 'on-site' ? 'bg-primary/20 text-primary' : 
                member.status === 'active' ? 'bg-green-500/10 text-green-500' :
                'bg-foreground/5 text-foreground/40'
              }`}>
                {member.status === 'on-site' ? t.onSite : member.status === 'active' ? t.active : 'Offline'}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-foreground/60 mt-1">
                {member.isInstaller ? <HardHat className="w-4 h-4" /> : <Shield className="w-4 h-4" />}
            ...
            {member.job_id && (
              <div className="mt-4 p-3 bg-surface/50 border border-border">
                <p className="text-[10px] font-mono text-foreground/40 uppercase mb-1">Current Assignment</p>
                <div className="text-sm font-semibold">{t.legacyId} {member.job_id}</div>
              </div>
            )}

            <div className="w-full mt-6 pt-5 border-t border-border flex justify-between items-center text-sm">
              <div className="flex gap-3">
                <button className="text-foreground/40 hover:text-primary transition-colors"><Phone className="w-4 h-4" /></button>
                <button className="text-foreground/40 hover:text-primary transition-colors"><Mail className="w-4 h-4" /></button>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-mono text-foreground/40 uppercase tracking-[0.2em]">{t.pin}:</span>
                <span className="font-mono bg-surface border border-border px-2 py-0.5 text-foreground/80">{member.pin}</span>
              </div>
            </div>
          </div>
        ))}
          ...
          <>
            ...
            >
              <div className="p-6 border-b border-border flex justify-between items-center bg-surface/30">
                <h2 className="text-xl font-black tracking-tight uppercase">{t.inviteMember}</h2>
                <button 
                  onClick={() => setIsInviteOpen(false)}
              ...
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
                <div>
                  <label className="block text-[10px] font-mono text-foreground/50 uppercase tracking-[0.2em] mb-2">Full Name</label>
                  <input type="text" className="w-full bg-surface/50 border border-border px-4 py-3 text-sm focus:outline-none focus:border-primary transition-colors" placeholder="e.g. John Doe" />
                </div>
                
                <div>
                  <label className="block text-[10px] font-mono text-foreground/50 uppercase tracking-[0.2em] mb-2">Email Address</label>
                  <input type="email" className="w-full bg-surface/50 border border-border px-4 py-3 text-sm focus:outline-none focus:border-primary transition-colors" placeholder="john@realstone.com" />
                </div>

                <div>
                  <label className="block text-[10px] font-mono text-foreground/50 uppercase tracking-[0.2em] mb-2">{t.role}</label>
                ...
                <div className="p-4 bg-surface/50 border border-border mt-2">
                  <h3 className="text-[10px] font-mono text-foreground/40 uppercase tracking-[0.2em] mb-1">Access PIN</h3>
                  <p className="text-xs text-foreground/60 leading-relaxed font-bold">
                    A secure 4-digit PIN will be automatically generated and sent to their email for field app access.
                  </p>
                </div>
              </div>

    ...
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-end">
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-mono text-rsg-gold uppercase tracking-widest font-black">Data Hydration Ready</span>
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-5 h-5 text-green-500" />
            <span className="text-xl font-black uppercase tracking-tight text-rsg-text">{parsedData.length} Jobs Detected</span>
          </div>
        </div>
        <button 
          onClick={() => { clearData(); setSelectedWoNumbers(new Set()); }}
          ...
          <thead>
            <tr className="bg-rsg-background border-b border-rsg-border">
              <th className="px-4 py-3 w-10">
                ...
                >
                  {selectedWoNumbers.size === parsedData.length && <div className="w-2 h-2 bg-rsg-gold" />}
                </div>
              </th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-rsg-text font-black">Status</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-rsg-text font-black">WO-ID</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-rsg-text font-black">Client</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-rsg-text font-black">Community</th>
              <th className="px-4 py-3 font-mono text-[10px] uppercase tracking-widest text-rsg-text font-black">Errors</th>
            </tr>
          </thead>
                ...
                >
                  <td className="px-4 py-3">
                    <div 
                      ...
                      className="w-4 h-4 border-2 border-rsg-border bg-rsg-surface flex items-center justify-center cursor-pointer transition-colors rounded-none"
                    >
                      {isSelected && <div className="w-2 h-2 bg-rsg-gold rounded-none" />}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    {isConflict ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <div className="group relative">
                          ...
                          <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 w-48 p-2 bg-rsg-surface border border-rsg-border shadow-xl opacity-0 group-hover:opacity-100 transition-opacity z-[60] pointer-events-none text-[9px] leading-tight font-medium uppercase tracking-wider text-rsg-text rounded-none">
                            This Work Order ID is already in the system. Selecting it will overwrite the existing record.
                          </div>
                        </div>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3 font-mono font-bold text-rsg-gold">{job.wo_number}</td>
                  <td className="px-4 py-3 font-bold uppercase tracking-tight text-rsg-text">{job.client_name}</td>
                  <td className="px-4 py-3 uppercase text-rsg-text/70">{job.community_name || 'N/A'}</td>
                  <td className="px-4 py-3">
                    {hasError && (

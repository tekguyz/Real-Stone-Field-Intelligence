...
export function ReportSignatureSection({ signatureUrl, jobId, updatedAt }: ReportSignatureSectionProps) {
  return (
    <div className="p-6 md:p-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 bg-surface print:bg-white break-inside-avoid">
      <div className="flex flex-col gap-2 w-full max-w-sm">
        <h3 className="font-black uppercase tracking-widest text-xs border-b border-border print:border-black/20 pb-2">Client Authorization</h3>
        {signatureUrl ? (
      ...
      <div className="flex flex-col items-end text-right">
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-foreground/40">Digital Authorization Token</span>
        <span className="font-mono text-xs text-foreground/80 print:text-black break-all max-w-[300px] uppercase">
          SIG-{jobId.substring(0,8)}-{Math.abs(jobId.split('').reduce((a,b)=>(((a<<5)-a)+b.charCodeAt(0))|0,0)).toString(16)}
        </span>
      </div>
    </div>
  );

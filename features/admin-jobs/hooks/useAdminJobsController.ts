...
export function useAdminJobsController() {
  ...
  const [selectedInstallerFilters, setSelectedInstallerFilters] = useState<string[]>([]);
  ...
  const cities = useMemo(() => 
    Array.from(new Set(currentJobs.map(j => j.address.split(',')[1]?.trim().split(' ')[0]).filter(Boolean))),
  [currentJobs]);
  
  const installers = useMemo(() => 
    Array.from(new Set(currentJobs.map(j => j.installer_id).filter((id): id is string => Boolean(id)))),
  [currentJobs]);

  const filteredJobs = useMemo(() => 
    currentJobs.filter(job => {
      const matchesSearch = job.client_name.toLowerCase().includes(search.toLowerCase()) || 
      ...
      const matchesCity = selectedCityFilters.length === 0 || (jobCity && selectedCityFilters.includes(jobCity));
      const matchesInstaller = selectedInstallerFilters.length === 0 || 
        (job.installer_id ? selectedInstallerFilters.includes(job.installer_id) : selectedInstallerFilters.includes('unassigned'));

      return matchesSearch && matchesStatus && matchesCity && matchesInstaller;
    }),
  [currentJobs, search, selectedStatuses, selectedCityFilters, selectedInstallerFilters]);

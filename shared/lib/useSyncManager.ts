...
export function useSyncManager() {
  ...
  const flushQueue = useCallback(async () => {
    ...
    if (pendingItems.length === 0) {
      ...
      setStatus(anyFailed ? 'OFFLINE' : (navigator.onLine ? 'ONLINE' : 'OFFLINE'));
      return;
    }
    ...
    for (const item of pendingItems) {
      ...
      try {
        if (item.action === 'UPLOAD_PHOTO') {
          ...
          await jobService.updateJobStatus(item.payload.jobId, item.payload.newStatus, isDevMode);
        }
    ...
    const remainingPending = allRemaining.filter(i => i.status !== 'failed_permanently').length;
    const permanentlyFailed = allRemaining.filter(i => i.status === 'failed_permanently').length;
    
    setPendingCount(remainingPending + permanentlyFailed);
    setStatus(permanentlyFailed > 0 ? 'OFFLINE' : (navigator.onLine ? 'ONLINE' : 'OFFLINE'));
